...
def _llm_chat(messages: list[dict], *, max_tokens: int, temperature: float = 0.3):
    ...
    出口处统一清洗所有 message 内容，避免 BOM / 控制符 / 替换字符
    进入 SDK 序列化（DashScope 兼容层偶发 ascii 编码崩溃的来源）。
    """
    clean_msgs = [
        {"role": m.get("role", "user"), "content": _clean(str(m.get("content", "")))}
        for m in messages
    ]
...
class LOLCollector(BaseCollector):
    ...
    def _bili_search(self, keyword: str, topn: int, retries: int = 3) -> list[dict]:
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                logger.info(f"B站搜索[{keyword}] 抓取 {len(out)} 条")
                return out
            except Exception as e:
                last_err = e
                logger.warning(f"B站搜索[{keyword}] 第{attempt}次失败（{e}），准备重试")
                if attempt < retries:
                    time.sleep(2 * attempt)  # 退避，换新时间戳重签
        raise last_err  # 全部重试失败 → 抛给 _fetch_bili 记 warning 兜底
